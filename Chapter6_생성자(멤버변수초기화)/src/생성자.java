...
/* 
 * 0. 클래스 설계
 * 	1) 변수 설정 : 데이터 저장 공간 생성 -> 인스턴스/static 변수
 * 	2) 변수 초기화 : 변수값 주입 -> 생성자(인스턴스), 초기화 블록(static)
 * 	3) 메소드 설정 : 저장된 데이터 활용(기능 설정) -> 인스턴스/static 메소드
 * 
 * 1. 생성자
 * 	- 클래스명(데이터형 매개변수) { 수행할 코드(인스턴스변수 초기화 코드) }
 *	- 객체 생성 시 '클래스명 객체명 = new 클래스명();'에서 new 뒤의 클래스명()이 생성자
 *								//new로 객체 생성 후 생성자로 초기화
 * 	1) 역할 
 * 		- 인스턴스변수 초기화 담당 메소드 : new로 생성된 메모리(인스턴스 heap)에 값 채워주는 역할
 * 		- 클래스 영역에서는 선언, 명시적 초기화만 가능, 구현 불가 -> 생성자 통해 진행
 * 			cf. 선언 : 변수 선언, 메소드 선언
 * 				구현 : 초기화된 데이터 변경, 외부/파일에서 데이터읽기, 메소드 호출, 제어문 사용, 연산자 사용
 * 						-> abstract void a(); //메소드 선언
 * 						   void a() { } //메소드 구현
 * 		- 객체마다 매개변수 통해 각각 다르게 초기화 가능
 ...
 * 								cf. random()은 입력값을 받는 것이 아니라 자동 생성이므로 기본생성자에 해당
 * 							 웹에서 사용 빈도 거의 없음
 * 
 * 2. this : 객체 자신을 가리키는 참조변수(객체 생성 시 객체의 주소값 저장)
 * 			cf. this()는 생성자. 서로 구분할 것
