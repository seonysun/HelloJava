 ...
 * 	- 1개의 클래스는 1개의 기능 수행하도록 구성
 * 
 * 	1) 데이터형 클래스 : 관련된 데이터(변수)를 모아서 관리 -> 사용자 정의 타입
 * 	2) 액션 클래스 : 관련된 기능(메소드) 모아서 관리 -> 메소드의 집합
 ...
 * 			cf. 객체 : (클래스에 설계된) 여러 데이터와 메소드를 저장할 수 있는 메모리 공간의 이름
 * 						//멤버 : 객체가 갖는 변수나 메소드 각각
 * 				변수 : 데이터 하나를 저장할 수 있는 메모리 공간의 이름
 * 				배열 : 같은 종류 데이터 여러 개를 저장할 수 있는 메모리 공간의 이름
 * 	- class 클래스명{
 * 			변수 설정 : 멤버변수(필드), 공유변수 -> 자동 초기화(0/null)
 * 			변수 활용 : 메소드
 * 			변수 초기화 : 생성자	}
 * 	- 클래스명 객체명 = new 클래스명();
 * 			-> 생성된 객체 주소가 객체명(참조변수)에 저장
 * 			-> 각각의 객체는 객체명(참조변수) 통해 다뤄짐
 * 		ex. Student를 정의, hong, lee, kang이라는 이름의 객체를 생성하여 사용
 * 	- 클래스명과 객체명은 식별자 생성 조건 적용
 * 	- 특징
 * 		- 코드 재사용성 높음, 코드 관리 용이
...
public class 클래스 {
	...
	public static void main(String[] args) {
		...
		System.out.println("나이:"+kang.age);
								//값 지정해주기 전에는 기본값(0/null)으로 자동초기화된 상태
