 ...
 * 1. 변수, 연산자, 제어문, 배열
 * 2. 클래스 
 * 	- 클래스 정의 방법 : [접근지정어] class 클래스명 { 
 * 							멤버변수(속성, 필드: instance) //변수는 배열, 클래스도 가능
 ...
 * 	- 특징
 * 		재사용 가능 -> 중복코딩 제거
 *		기존의 클래스 확장해서 새로운 클래스 제작 -> 유지보수 용이
 * 		단일 상속만 가능
 * 		데이터형 : 상위클래스 > 하위클래스 -> 상속이 없는 클래스의 크기는 동일, 상속이 있는 경우에만 크기 비교 가능
 * 		메모리 : 상위클래스 < 하위클래스
 * 	- 메모리 할당(참조변수로 생성, 생성자로 받음) -> 참조변수 타입에 따라 접근 가능한 멤버의 개수 달라짐(다형성)
 * 		하위 클래스로 생성 Sub s=new Sub(); 
 * 		상위 클래스로 생성, 하위 클래스로 받음 Super s=new Sub(); //추상클래스, 인터페이스(구현된 메소드가 없으므로 하위클래스의 생성자 활용)
 * 		하위 클래스로 생성, (형변환)상위 클래스로 받음 Sub s=(Sub)new Super(); //라이브러리
 * 	- super : 상위클래스 제어 
 ...
 * 	- 차이점
 * 		목적) 추상클래스 : 상속 받아서 확장
 * 			 인터페이스 : 관련된 여러 개의 클래스 묶어서 관리(메소드 동일)
 * 		-----------------------------------------------
 * 				   추상클래스 			  인터페이스
 * 		-----------------------------------------------
 * 		상속		   단일 상속			  다중 상속
