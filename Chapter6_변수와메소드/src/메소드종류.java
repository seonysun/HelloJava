 ...
 * 0. 클래스 : 한 개의 큰 기능
 * 	  메소드 : 세부 기능 각각
 * 		ex. 게시판 기능의 클래스 -> 글쓰기 메소드, 삭제 메소드, 수정 메소드, 목록출력 메소드, 상세보기 메소드, 
 * 
 * 1. 메소드의 종류
 ...
 * 			-> 클래스 메소드는 자동으로 저장되므로 인스턴스 메소드 호출 위해서는 동적메모리 할당 필요
 * 			   (클래스의 객체를 생성하면 메소드의 동적메모리 할당됨)
 * 		  인스턴스 메소드 안에서는 인스턴스/클래스 메소드 모두 호출 가능
 * 			-> 인스턴스 메소드는 바로 저장되는 것이 아니라 클래스로 동적메모리 할당 후 저장되므로 상관없음
 ...
 *				- 기본형, String : Call By Value; 실제 값으로 메소드에 전달
 *			  					-> 매개변수의 메모리 저장 공간에 실제 변수의 리터럴이 저장
 *									=> 메소드 수행 시 값이 변경되더라도 실제 변수값은 그대로 유지
 *								 read only
 *				- 참조형(배열, 클래스) : Call By Reference; 주소로 메소드에 전달
 *									-> 매개변수의 메모리 저장 공간에 실제 변수가 저장된 주소값이 저장
 ...
 * 
 * 	3) static : static 리턴형 메소드명(매개변수) {	return 값; }
 * 
 * 3. 메소드 호출
