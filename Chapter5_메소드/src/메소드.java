 ...
 *  							ex. 로그인 여부 확인 -> boolean 타입의 결과값 도출
 *  							cf. 리턴형은 결과값보다 큰 데이터형도 가능하지만 특별한 경우가 아니면 동일하게 통일함
 *  							cf. 결과값은 1개만 설정 가능(return은 여러번 처리 가능)
 *  									-> 여러 개일 경우 배열이나 클래스 하나로 출력
 *  					- 결과값이 메소드 내에서 자체적으로 처리되어 넘겨줄 결과값이 없는 경우 : void로 설정
 * 							-> return(넘겨줄 결과값) 생략 가능(컴파일러가 자동으로 return; 추가하여 진행)
 * 								ex. 데이터베이스 => insert, update, delete ; 따로 결과값 넘겨줄 필요 없이 DB 내에서 처리되고 끝
 * 								ex. 구구단 출력 메소드(출력 과정 자체가 메소드 내에 포함)
 * 						- 타입 예시
 * 							(1) 나누기 값 : double
 * 							(2) 영화 제목 : String[]
 * 							(3) 경우의 수 2개 : boolean
 ...
 * 					ex. printf
 * 		- static : 자동 메모리, static 없으면 new 이용해서 따로 공간 생성해야 함
 * 
 * 	4) 메소드 호출
 * 		- 메소드명(argument);
