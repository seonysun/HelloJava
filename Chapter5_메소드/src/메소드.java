 ...
 * 		- 구조화된 프로그램(가독성 좋음) : 수정 용이(메소드 하나만 처리하면 연결된 소스 자동 반영)
 ...
 * 								   void //결과값이 없는 상태
 * 			  X			  X		=> System.out.println(); 출력물 없이 다음 줄에 출력하라는 정보만 전달
 * 
 * 	3) 메소드 정의
 * 		- 선언부(원형) + 구현부
 ...
 * 								cf. return 생략 가능(컴파일러가 자동으로 추가하여 처리)
 * 						- 타입 예시
 * 							(1) 나누기 값 : double
 * 							(2) 영화 제목 : String[]
 * 							(3) 경우의 수 2개 : boolean
 ...
 * 				   - 웹에서의 검색, 입력, 버튼 출력, 메뉴 클릭, 페이지 요청, 상세보기
 * 				   - 없는 경우 공백으로 처리 ()
